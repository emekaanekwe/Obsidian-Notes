...
## Question 2
Let $f(w)= 3w^2 - 4w + 10$. Assume that we use gradient descent with the learning rate $η=0.05$ to solve $min_w f(w)$. At the iteration t, we are at $w_t=2$. What is the value of $w_t+1$ at the next iteration?
Select one:
a.
1.6
b.
...
$$\stackrel{min}{\theta}L(D;\theta):=\frac{1}{N}\sum^N_{i=1}\mathit{l}(x_i,y_i;\theta)$$
...
## Question 10
How to train a denoising auto-encder with encoder $f_{\theta}$ and decoder $g_{\theta}$? Consider the following diagram:
...
b.
$$\underset{\theta, \phi}{min}E_{x \sim P}[E_{\epsilon \sim N(0,\eta I)}[d(x, f_{\theta}(g_{\phi}(x+\epsilon)))]$$
c.
$$\underset{\theta, \phi}{min}E_{x \sim P}[E_{x'\sim N(x,\eta I)}[d(x, g_{\phi}(f_{\theta}(x+\epsilon)))]$$
d.
$$\underset{\theta, \phi}{min}E_{x\sim P}[E_{x'\sim N(x,\eta I)}[d(x, f_{\theta}g_{\phi}(x')))]$$

## Question 11
Which are the correct expressions of how to train GANs?
...
**Sequence 1** (all timesteps): $$\begin{bmatrix} 1 & -3 \ -4 & 2 \ 1 & -3 \ -1 & 3.5 \ 1 & -3 \end{bmatrix}$$

**Sequence 2** (all timesteps): $$\begin{bmatrix} -4 & 2 \ 1 & -3 \ 1 & -1 \ -4 & 2 \ -4 & 2 \end{bmatrix}$$
## Question 16
...
$$score(h_t,\bar h_s)=h^T_t \cdot \bar h_s =(h_t,\bar h_s)\ \text{for }\ s=1,2,3$$
$$a_t (s)=\frac{exp(score(h_t, \bar h_s))}{\sum^3_{i=1}exp(score(h_t,\bar h_i))}\ \text{for }\ s=1,2,3$$
...
$score(h_t,\bar h_1)=sign(h_t,\bar h_1)=1$
$score(h_t,\bar h_2)=sign(h_t,\bar h_2)=-1$
$score(h_t,\bar h_3)=sign(h_t,\bar h_3)=1$
$a_t(1)=\frac{e}{e+e+e^{-1}}=0.47$
$a_t(2)=\frac{e}{e+e+e^{-1}}=0.06$
$a_t(3)=\frac{e}{e+e+e^{-1}}=0.47$
$c_t=\sum^3_{s=1}a_t(s)\bar h_s=1.35$
